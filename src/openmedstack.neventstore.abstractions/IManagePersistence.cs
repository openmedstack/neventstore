...
public interface IManagePersistence
{
    ...
    IAsyncEnumerable<ICommit> GetFrom(string TenantId, long checkpointToken, CancellationToken cancellationToken);
    ...
    /// <exception cref="StorageException" />
    /// <exception cref="StorageUnavailableException" />
    IAsyncEnumerable<IStreamHead> GetStreamsToSnapshot(string TenantId, int maxThreshold, CancellationToken cancellationToken);
    ...
    /// </summary>
    Task<bool> Purge(string TenantId);
...
//    /// Deletes a stream.
//    /// </summary>
//    /// <param name="TenantId">The bucket Id from which the stream is to be deleted.</param>
//    /// <param name="streamId">The stream Id of the stream that is to be deleted.</param>
//    Task<bool> DeleteStream(string TenantId, string streamId);
}
